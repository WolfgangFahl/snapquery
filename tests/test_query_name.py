...
from snapquery.snapquery_core import NamedQueryManager, QueryNameSet
...
class TestQueryName(Basetest):
    ...
    def test_query_name_set(self):
        ...
        with tempfile.NamedTemporaryFile() as tmpfile:
            ...
            qimport.import_samples(with_store = True,
            ...
            test_cases = [
                ...
                (("wikidata.org", "examples"), (302, 1, 1, 5)),  # Specific to 'wikidata.org' and 'examples'
                (("", "examples"), (309, 3, 1, 7)),  # Any domain with 'examples' namespace
                (("x-invalid", ""), (0, 0, 0, 0)),  # No results expected for an invalid domain
            ]
    
            for (domain, namespace), (expected_total, expected_domains_count, expected_namespaces_count, expected_names_count) in test_cases:
                with self.subTest(domain=domain, namespace=namespace):
                    qns.update(domain, namespace)
                    if self.debug:
                        print(qns)
                    self.assertGreaterEqual(qns.total, expected_total, f"Total queries should be at least {expected_total}")
                    self.assertGreaterEqual(len(qns.domains), expected_domains_count, f"Number of domains should be at least {expected_domains_count}")
                    self.assertGreaterEqual(len(qns.namespaces), expected_namespaces_count, f"Number of namespaces should be at least {expected_namespaces_count}")
                    self.assertGreaterEqual(len(qns.names), expected_names_count, f"Number of names should be at least {expected_names_count}")
